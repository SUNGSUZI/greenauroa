		            ...
		            <tbody>
		                ...
		                <c:forEach items="${qnaList}" var="qna" varStatus="status">
		                    <tr>
								...
								<c:choose>
								    <c:when test="${qna.secret == 'Y' && (principal != null && (principal.authorities[0].authority == 'ROLE_ADMIN' || qna.memberId == principal.name))}">
								        <a href="/qna/qdetail/${qna.boardKey}">${qna.title} <img class="list_lock" src="/images/icon8-lock.png"></a>
								    </c:when>
								
								    <c:when test="${qna.secret == 'Y' && (principal == null || principal.authorities.isEmpty() || (principal.authorities[0].authority != 'ROLE_ADMIN' && qna.memberId != principal.name))}">
								        <span>${qna.title} <img class="list_lock" src="/images/icon8-lock.png"></span>
								    </c:when>
